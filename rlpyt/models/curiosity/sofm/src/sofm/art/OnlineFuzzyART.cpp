...
namespace sofm
{
    ...
    namespace art
    {
        ...
        std::vector<int> OnlineFuzzyART::run_online(const MatrixConstRef features, int max_epochs)
        {
            ...
            while (!w_.isApprox(w_old) && epoch < max_epochs)
            {
                w_old = w_;
        ...
        int OnlineFuzzyART::train_pattern(const VectorConstRef pattern)
        {
            ...
            w_.row(winner) = beta_ * utils::fuzzy_and(pattern, w_.row(winner)).transpose() + (1.0 - beta_) * w_.row(winner);
