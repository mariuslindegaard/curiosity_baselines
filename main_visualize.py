...
"""Frontends for agents who want to play pycolab games."""
...
def observation_to_tensor(observation):
    return torch.tensor(observation.layers.values()).float()


class AgentUi(object):
    """A terminal-based UI for pycolab games."""

    def __init__(self, agent, delay=None,
    ...
    def _init_curses_and_play(self, screen):
        ...
        if self._delay is None:
            ...
            # Nonblocking (if 0) or timing-out reads
    ...
    def _display(self, screen, observations, score, elapsed):
        ...
        for observation in observations:
            ...
            for row, board_line in enumerate(observation.board, start=1):
                # Move to start of this board row.
                screen.move(row, leftmost_column)
    ...
    def _init_colour(self):
        ...
        # We use colour IDs from large to small.
...
from rlpyt.envs.mazeworld.mazeworld.envs.mazeworld_env import OrdealEnv
import gym
...
def deepmind_make_mini(game='DeepmindOrdealEnv-v0', obs_type='mask', max_iterations=500):
    import rlpyt.envs.mazeworld.mazeworld

    env = gym.make(game, obs_type=obs_type, max_iterations=max_iterations)
    logdir = './results/ppo_Deepmind5RoomBouncing-v0/run_0'
    log_heatmaps = False
    env.pycolab_init(logdir, log_heatmaps)
...
if __name__ == "__main__":
    ...
    model_kwargs = {'curiosity_kwargs': {'curiosity_alg': 'rnd', 'std_rew_scaling': 1.0, 'feature_encoding': 'idf_maze', 'frame_stacking': False, 'maze_environment': True, 
                                         'prediction_beta': 1.0, 'drop_probability': 1.0, 'batch_norm': False, 'gamma': 0.99, 'device': 'cpu'}}
    no_extrinsic = False

    agent = AtariLstmAgent(
        initial_model_state_dict=initial_model_state_dict,
        model_kwargs=model_kwargs,
        no_extrinsic=no_extrinsic
    )
    ordeal_env = deepmind_make_mini(game='DeepmindOrdealEnv-v0', obs_type='mask', max_iterations=500)

    agent.initialize(ordeal_env.spaces, share_memory=True, global_B=1, obs_stats=None, env_ranks=[0])

    agent_ui = AgentUi(agent)

    agent_ui.play(ordeal_env)
